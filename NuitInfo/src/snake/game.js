...
var Game = {
    ...
    create: function () {
        ...
        for (var i = 0; i < 10; i++) {
            ...
            var segmentColor = i === 9 ? 'rgba(0, 204, 204, 0.8)' : 'rgba(0, 204, 204, 0.8)'; // Head is more rusty
            snake[i].beginFill(segmentColor, 0.9);
            snake[i].drawRect(0, 0, squareSize, squareSize);
            snake[i].endFill();
            // Add rusty border
            snake[i].lineStyle(1, 'rgba(0, 204, 204, 0.8)', 0.8);
            snake[i].drawRect(0, 0, squareSize, squareSize);
            // Add metal shine effect
            snake[i].beginFill('rgba(0, 204, 204, 0.8)', 0.3);
